...
var dbfn = path.join(__dirname, argv1);
...
for(var i = 0; i < db.chapters.length; ++i) {
	if(!db.chapters[i].hasOwnProperty('duration')) {
		...
		var rv = child_process.spawnSync('ffprobe', [ '-i', path.join(_dbdir, fn) ], { stdio: [ 'ignore', 'pipe', 'pipe'], encoding: 'utf8'});
...
if(!db.hasOwnProperty('destination')) {
	...
	var rv = child_process.spawnSync('ffmpeg', [ '-f', 'concat', '-i', filelist, '-codec', 'copy', '-movflags', '+faststart', path.join(_dbdir, dst) ], { stdio: [ 'ignore', 'pipe', 'pipe']});
...
if(read_intervals.length > 0) {
	var rv = child_process.spawnSync('ffprobe', [ '-i', db.destination, '-print_format', 'csv', '-show_frames', '-read_intervals', read_intervals.join(',') ], { stdio: [ 'ignore', 'pipe', 'pipe'], encoding: 'utf8'});
	if(!rv || rv.error || rv.status) {
		console.log(rv);
		process.exit(1);
	}
...
if(!db.hasOwnProperty('chaptered') || !db.chaptered) {
	...
	var dst = path.join(_dbdir, db.destination);
	var tvar = String((new Date()).valueOf());
	var tmp4 = path.join(_dbdir, `tmp-${tvar}.mp4`);
	var rv = child_process.spawnSync('mp4box', [ '-ipod', '-add', `${dst}#video`, '-add', `${dst}#audio`, '-add', `${chapttxt}:chap`, tmp4], { stdio: [ 'ignore', 'pipe', 'pipe'] });
...
if(!db.hasOwnProperty('finished') || !db.finished) {
	...
	if(db.artwork) {
		args.push('--artwork');
		args.push(path.join(_dbdir, db.artwork));
	}
...
// Done.
fs.writeFileSync(path.join(dbfn), JSON.stringify(db, null, '\t'));
process.exit(0);
